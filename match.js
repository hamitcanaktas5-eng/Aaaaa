...
function renderStats() {
  if (!match.stats.length) {
    document.getElementById('tab-content').innerHTML = emptyHtml('📊', 'Veri Yok', 'Maç başladığında istatistikler görünecek.');
    return;
  }
...
function renderLineup() {
  ...
  const hasData = hlu.starting.length > 0 || alu.starting.length > 0;
